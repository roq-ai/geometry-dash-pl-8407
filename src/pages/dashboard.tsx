...
import { getNewTableZxqos } from 'apiSdk/new-table-zxqos';
import { NewTableZxqoInterface } from 'interfaces/new-table-zxqo';
import { getNewTables } from 'apiSdk/new-tables';
import { NewTableInterface } from 'interfaces/new-table';
import { getNewTableIwfbs } from 'apiSdk/new-table-iwfbs';
import { NewTableIwfbInterface } from 'interfaces/new-table-iwfb';

import { NewTableZxqoListPage as ShowCaseList } from 'pages/new-table-zxqos';

function DashboardPage() {
  const { session } = useSession();

  const newTableZxqoStat = useSWR<PaginatedInterface<NewTableZxqoInterface>>(
    () => `/new-table-zxqos`,
    () => getNewTableZxqos({ limit: 1, offset: 0 }),
  );
  ...
  const newTableIwfbStat = useSWR<PaginatedInterface<NewTableIwfbInterface>>(
    () => `/new-table-iwfbs`,
    () => getNewTableIwfbs({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'New Table Zxqos',
      result: newTableZxqoStat?.data?.totalCount,
      isLoading: newTableZxqoStat.isLoading,
    ...
    {
      title: 'New Table Iwfbs',
      result: newTableIwfbStat?.data?.totalCount,
      isLoading: newTableIwfbStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest New Table Zxqos
                </Text>
                <NextLink href={`new-table-zxqos`} passHref legacyBehavior>
                  <Button
